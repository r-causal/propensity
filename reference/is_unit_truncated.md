...
fixed set of observations and can both be lost and outlive them:
...
that grows the vector carries it across a length change.
`is_unit_truncated()` therefore checks that the record covers the object
it is given, and raises an error of class
`propensity_missing_meta_error` when it does not, rather than name
truncated units at stale positions.
...
answers from it and names the wrong units. See
