...
order, with one exception: the `contrast` column that names the pair of
exposure levels a row compares belongs to the marginal reading of a
categorical result alone, and that result's conditional reading returns
the same table one column narrower.
