...
# a density does not reach the weights through a modified score
...
# censoring weights do not carry a density through a modified score
...
---
    ...
    Condition <propensity_numerator_error>
      ...
      i Leave `.sigma` unset to spread the conditional density by the estimator its family asks for, or use `numerator` = "marginal", which takes a spread you supply.
...
# stabilize takes one of the answers or a model and nothing else
    ...
    Condition <propensity_stabilize_error>
      ...
      x It is a string.
      i A fitted model stabilizes the weights on the numerator it estimates: a conditional density for a dose, the conditional probability of the level each unit took for a binary exposure, and that same probability read across every level for a categorical one. To stabilize on a numerator you computed yourself, set `stabilize = TRUE` and pass it as `stabilization_score`.
