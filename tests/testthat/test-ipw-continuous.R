...
#
# `se_method = "bootstrap"` resamples the whole fit rather than stacking it, so
# it reaches the two continuous routes the sandwich has no equation for, an
# additive propensity score model and weights built with a kernel density. Each
# replicate resamples the rows of `.data`, refits the propensity score model,
# rebuilds the weights from the record the supplied weights carry, refits the
# marginal structural model with them, and keeps the coefficients the result
# reports. The point estimate is the one the supplied models already produced;
# only the spread is resampled.

# The number of replicates the smoke tests run. It clears the fifty successful
# replicates the method insists on with room to spare and keeps the suite quick;
# the one test that compares a bootstrap standard error against the sandwich
# runs enough replicates for the comparison to mean something.
boot_reps_smoke <- 60L

test_that("ipw() continuous bootstrap reports the MSM coefficient and a resampled spread", {
  skip_if_not_installed("deli")
  skip_on_cran()
  ...
  expect_s3_class(res, "ipw")
  expect_identical(res$se_method, "bootstrap")
  ...
  # structural model the user fit already reports.
  expect_equal(
    res$estimates$estimate,
    unname(coef(mods$outcome_mod)[["A"]]),
    tolerance = 1e-8
  ...
  # The resampled spread and the sandwich are estimating the same quantity, so
  # they agree to within the finite-sample and Monte Carlo gap. Two hundred
  # replicates on this fixture sit near four percent apart; the band is wide
  # enough that a passing test says the two methods agree rather than that the
  # seed was lucky.
  mest_se <- ipw(mods$ps_mod, mods$outcome_mod)$estimates$std.err
  rel <- abs(res$estimates$std.err - mest_se) / mest_se
  expect_lt(rel, 0.15)
})

test_that("ipw() continuous bootstrap builds its table from the replicates it kept", {
  skip_if_not_installed("deli")
  skip_on_cran()
  ...
  res <- ipw(
    ...
    .data = dat,
    se_method = "bootstrap",
    boot_reps = boot_reps_smoke,
    boot_seed = 1L
  )

  reps <- res$fit$reps
  expect_true(is.matrix(reps))
  expect_identical(ncol(reps), nrow(res$estimates))
  expect_true(all(is.finite(reps)))

  # Every number in the table is a function of the replicates, and each of these
  # says which one. The columns of the replicate matrix may carry the labels the
  # table's rows carry, so the comparisons are made on the values alone.
  expect_equal(res$estimates$std.err, unname(apply(reps, 2, stats::sd)))
  expect_equal(res$estimates$z, res$estimates$estimate / res$estimates$std.err)
  expect_equal(
    res$estimates$p.value,
    2 * stats::pnorm(abs(res$estimates$z), lower.tail = FALSE)
  )

  # The interval is the normal approximation around the point estimate, which is
  # the interval `tidy()` and `pool_ipw()` already read from every other route.
  crit <- stats::qnorm((1 + 0.95) / 2)
  expect_equal(
    res$estimates$ci.lower,
    res$estimates$estimate - crit * res$estimates$std.err
  )
  expect_equal(
    res$estimates$ci.upper,
    res$estimates$estimate + crit * res$estimates$std.err
  )

  # The covariance of the reported rows, stored the way the stacked routes store
  # theirs, keyed by the labels the table's rows carry.
  vc <- attr(res$estimates, "ipw_vcov")
  expect_true(is.matrix(vc))
  expect_identical(dim(vc), rep(nrow(res$estimates), 2L))
  expect_identical(dimnames(vc), list("slope", "slope"))
  expect_equal(unname(vc), unname(stats::cov(reps)))
})

test_that("ipw() continuous bootstrap stores a fit that records the resampling", {
  skip_if_not_installed("deli")
  skip_on_cran()
  ...
  expect_s3_class(res$fit, "ipw_bootstrap")
  expect_true(
    all(
      c("reps", "seed", "n_reps", "n_failed", "vcov", "percentile") %in%
        names(res$fit)
    )
  )

  # `n_reps` is the number of replicates asked for and `n_failed` the number
  # dropped, so the rows kept are what is left of the request.
  expect_identical(res$fit$n_reps, boot_reps_smoke)
  expect_identical(res$fit$n_failed, 0L)
  expect_identical(nrow(res$fit$reps), boot_reps_smoke - res$fit$n_failed)
  expect_identical(res$fit$seed, 1L)
  expect_equal(res$fit$vcov, attr(res$estimates, "ipw_vcov"))

  # Percentile bounds are reported alongside the normal-approximation interval
  # rather than in place of it: one row per reported effect, a lower and an upper
  # bound at the confidence level asked for.
  pct <- res$fit$percentile
  expect_true(is.matrix(pct))
  expect_identical(dim(pct), c(nrow(res$estimates), 2L))

  # The bounds are quantiles of the replicates rather than any particular
  # quantile definition, so they are pinned by what every definition agrees on:
  # each bound lies inside the range the replicates span, and the pair brackets
  # the middle of them with an interval of some width.
  reps <- unname(res$fit$reps[, 1])
  expect_true(all(pct[1, ] >= min(reps) & pct[1, ] <= max(reps)))
  expect_lt(pct[1, 1], stats::median(reps))
  expect_gt(pct[1, 2], stats::median(reps))
  expect_true(all(pct[, 1] < pct[, 2]))
})

test_that("ipw() continuous bootstrap wraps the outcome model and leaves wt_mod alone", {
  skip_if_not_installed("deli")
  skip_on_cran()
  ...
  # The outcome model carries the bootstrap covariance of all of its
  # coefficients, the intercept included, so the conditional reading and
  # `vcov()` report the resampled spread rather than the one least squares
  # reports for a weighted fit.
  expect_s3_class(res$outcome_mod, "ipw_model")
  coef_names <- names(coef(mods$outcome_mod))
  vc <- vcov(res$outcome_mod)
  expect_identical(dim(vc), rep(length(coef_names), 2L))
  expect_identical(dimnames(vc), list(coef_names, coef_names))
  expect_equal(vc[["A", "A"]], res$estimates$std.err^2)
  expect_false(isTRUE(all.equal(vc, vcov(mods$outcome_mod))))

  # The propensity score model carries no block of anything here, because
  # resampling solves no system the two models share, so it is stored as it was
  # supplied.
  expect_identical(res$wt_mod, mods$ps_mod)
  expect_false(inherits(res$wt_mod, "ipw_model"))
})

# ---- what the bootstrap needs and what it refuses ----------------------------

test_that("ipw() refuses boot_reps and boot_seed under the other standard error methods", {
  skip_if_not_installed("deli")
  dat <- sim_continuous()
  mods <- fit_continuous_models(dat)

  # The two arguments describe resampling, so naming either under a method that
  # resamples nothing is a request that cannot be honored rather than one that
  # is quietly ignored.
  expect_error(
  ...
  expect_error(
    ipw(mods$ps_mod, mods$outcome_mod, .data = dat, boot_seed = 1L),
    class = "propensity_unsupported_arg_error"
  )
  expect_error(
    ipw(
...
test_that("ipw() refuses the bootstrap for a binary exposure", {
  skip_if_not_installed("deli")
  ...
  # This pass builds the bootstrap for the continuous route alone. A binary
  # exposure is refused for the method it does not have rather than routed into
  # a resampling loop that was never written for it.
  err <- expect_error(
  ...
  # The two resampling arguments are refused on the binary route as well, under
  # whichever method a binary exposure was going to use.
  expect_error(
    ipw(
      ps_mod,
      outcome_mod,
...
test_that("ipw() continuous bootstrap requires .data", {
  skip_if_not_installed("deli")
  dat <- sim_continuous()
  mods <- fit_continuous_models(dat)
  ...
  # The wording is pinned by a snapshot in the green round; what this asserts is
  # that the refusal names the argument the caller has to supply.
  expect_match(
    gsub("[[:space:]]+", " ", conditionMessage(err)),
    ".data",
    fixed = TRUE
...
test_that("ipw() continuous bootstrap refuses weights built with an observation-level `.sigma`", {
  skip_if_not_installed("deli")
  dat <- sim_continuous()
  ps_mod <- lm(A ~ x1 + x2, data = dat)
  sigma_wts <- wt_ate(
  ...
  # A spread supplied for each observation is a function of the data that no
  # refit reproduces, so the replicate would rebuild weights the user never
  # built. The refusal is the one the stacked route already makes.
  expect_error(
    ipw(
      ps_mod,
      outcome_mod,
...
test_that("ipw() continuous bootstrap refuses .by", {
  skip_if_not_installed("deli")
  dat <- sim_continuous()
  mods <- fit_continuous_models(dat)
  ...
  expect_true(
    any(
      c(
        "propensity_ipw_by_method_error",
        "propensity_ipw_by_exposure_error"
      ) %in%
        class(err)
    )
  )
...
# ---- the seed the replicates are drawn under --------------------------------

test_that("ipw() continuous bootstrap is reproducible under boot_seed", {
  skip_if_not_installed("deli")
  skip_on_cran()
  ...
  expect_identical(first$fit$reps, second$fit$reps)

  # A different seed draws different resamples, so the spread moves while the
  # point estimate, which is not resampled, does not.
  other <- boot(8L)
  expect_identical(other$estimates$estimate, first$estimates$estimate)
  expect_false(identical(other$estimates$std.err, first$estimates$std.err))
})

test_that("ipw() continuous bootstrap draws from the current RNG state when boot_seed is NULL", {
  skip_if_not_installed("deli")
  skip_on_cran()
  ...
  # No seed of its own means the caller's seed decides, so a session seeded
  # outside the call reproduces the result the way it reproduces any other
  # random draw.
  first <- withr::with_seed(11, boot())
  second <- withr::with_seed(11, boot())
  expect_identical(first$fit$reps, second$fit$reps)
  expect_identical(first$estimates, second$estimates)
  expect_null(first$fit$seed)
})

# ---- the routes the sandwich has no equation for ----------------------------

test_that("ipw() bootstraps a gam propensity score model end to end", {
  skip_if_not_installed("mgcv")
  skip_if_not_installed("deli")
  skip_on_cran()

  dat <- sim_continuous(n = 400)
  ps_mod <- mgcv::gam(A ~ s(x1) + x2, data = dat, family = gaussian())
  wts <- continuous_weights(as.double(fitted(ps_mod)), dat$A)
  msm <- lm(yc ~ A, data = dat, weights = wts)
...
test_that("ipw() bootstraps kernel-density weights end to end", {
  skip_if_not_installed("deli")
  skip_on_cran()
...
# ---- a dose-response curve under the bootstrap ------------------------------

test_that("ipw() bootstraps a dose-basis MSM and keeps the conditional reading", {
  skip_if_not_installed("deli")
  skip_on_cran()
  ...
  ps_mod <- lm(A ~ x1 + x2, data = dat)
  wts <- continuous_weights(as.double(fitted(ps_mod)), dat$A)
  msm <- lm(yc ~ poly(A, 2), data = dat, weights = wts)
  ...
  # The reading a curve has is a property of the fit rather than of how the
  # variance was computed, so resampling reports the same one row per basis
  # column the stacked route reports.
  expect_identical(res$effects, "conditional")
  expect_identical(res$readings, "conditional")
  expect_named(
    res$estimates,
    c("effect", "contrast", estimates_columns[-1L])
  )
  expect_identical(nrow(res$estimates), 2L)
  expect_identical(res$estimates$effect, rep("coef", 2L))
  expect_identical(res$estimates$contrast, c("poly(A, 2)1", "poly(A, 2)2"))
  expect_equal(
    res$estimates$estimate,
    unname(coef(msm)[c("poly(A, 2)1", "poly(A, 2)2")]),
    tolerance = 1e-8
  )
  expect_identical(ncol(res$fit$reps), 2L)

  # The conditional surface is the outcome model's own coefficient vector under
  # the bootstrap covariance, and the marginal reading is refused from the same
  # place it is refused on the stacked route.
  expect_identical(tidy(res, effects = "conditional")$term, names(coef(msm)))
  expect_error(
    ipw(
      ps_mod,
      msm,
...
# ---- replicates that fail ---------------------------------------------------
#
# A replicate is dropped when its refits error or leave a coefficient that is
# not finite. Both are counted, and the count decides between a result that says
# how much of the request survived and no result at all. Deterministic failures
# are hard to simulate from data alone, so the count is driven through
# `ipw_boot_once()`, the internal that runs one replicate: it takes the row
# indices the replicate resamples and returns the reported outcome coefficients,
# and a return holding non-finite values is a failed replicate.

test_that("ipw() continuous bootstrap warns when more than five percent of replicates fail", {
  skip_if_not_installed("deli")
  skip_on_cran()
  ...
  expect_warning(
    ...
    class = "propensity_ipw_bootstrap_warning"
  )

  expect_identical(res$fit$n_reps, 100L)
  expect_identical(res$fit$n_failed, 20L)
  expect_identical(nrow(res$fit$reps), 80L)
  expect_true(is.finite(res$estimates$std.err))
})

test_that("ipw() continuous bootstrap keeps quiet when few replicates fail", {
  skip_if_not_installed("deli")
  skip_on_cran()
  ...
  # One failure in a hundred is below the threshold, so the result reports the
  ...
  expect_identical(res$fit$n_failed, 1L)
  expect_identical(nrow(res$fit$reps), 99L)
})

test_that("ipw() continuous bootstrap errors when fewer than fifty replicates succeed", {
  skip_if_not_installed("deli")
  skip_on_cran()
  ...
  # Fifty successful replicates is the floor, and a request below it cannot
  # clear the floor however well every replicate goes, so the standard error is
  # refused rather than reported from too few draws.
  err <- expect_error(
    ipw(
      mods$ps_mod,
      mods$outcome_mod,
    ...
    class = "propensity_ipw_bootstrap_error"
  )
  expect_match(
    gsub("[[:space:]]+", " ", conditionMessage(err)),
    "50",
