...
test_that("ipw() refuses the bootstrap on the two-model route", {
  skip_if_not_installed("deli")
  dat <- sim_joint_models()
  two <- fit_joint_models_route(dat)

  # Resampling rebuilds the weights of one propensity score model of a
  # continuous exposure from the record they carry. The product of two
  # treatment models' weights is not that record, so the request is refused by
  # name rather than routed into a loop that would rebuild the wrong thing.
  err <- expect_error(
    ipw(two$models, two$outcome_mod, .data = dat, se_method = "bootstrap"),
...
test_that("ipw() refuses boot_reps and boot_seed on the two-model route", {
  skip_if_not_installed("deli")
  dat <- sim_joint_models()
  two <- fit_joint_models_route(dat)

  expect_error(
    ipw(two$models, two$outcome_mod, boot_reps = 100L),
    class = "propensity_unsupported_arg_error"
  )
  expect_error(
    ipw(two$models, two$outcome_mod, boot_seed = 1L),
    class = "propensity_unsupported_arg_error"
  )
  expect_propensity_error(
    ipw(two$models, two$outcome_mod, boot_reps = 100L)
