test_that("ps_trunc works with matrix propensity scores", {
  ...
  truncated <- ps_trunc(ps_matrix, .exposure = exposure, method = "ps", lower = 0.05)
...
test_that("ps_trunc errors for unsupported methods with categorical", {
  ...
  expect_error(
    ps_trunc(ps_matrix, .exposure = exposure, method = "pctl"),
    "must be one of"
  )
...
test_that("is_unit_truncated works for matrix objects", {
  ...
  truncated <- ps_trunc(ps_matrix, .exposure = exposure, method = "ps", lower = 0.1)
...
test_that("ps_trunc handles extreme values correctly", {
  ...
  truncated <- ps_trunc(ps_matrix, .exposure = exposure, method = "ps", lower = 0.01)
...
test_that("ps_trunc.ps_trunc warns about already truncated scores", {
  ...
  truncated_once <- ps_trunc(ps_matrix, .exposure = exposure, method = "ps", lower = 0.05)
  
  expect_warning(
    truncated_twice <- ps_trunc(truncated_once, .exposure = exposure, method = "ps", lower = 0.1),
...
test_that("ps_trunc matches PSweight truncation behavior for entropy weights", {
  ...
  ps_matrix[1:30, ] <- matrix(rep(c(1e-7, 0.5, 0.5 - 1e-7), each = 30), ncol = 3, byrow = TRUE)
  ps_matrix[31:60, ] <- matrix(rep(c(0.3, 0.3, 0.4), each = 30), ncol = 3, byrow = TRUE)
  ps_matrix[61:100, ] <- matrix(rep(c(0.99999, 5e-6, 5e-6), each = 40), ncol = 3, byrow = TRUE)
  ...
  truncated <- ps_trunc(ps_matrix, .exposure = exposure, method = "ps", lower = 1e-6)
...
test_that("categorical truncation preserves relative proportions", {
  ...
  ps_matrix <- matrix(
    c(0.001, 0.599, 0.4,    # First value needs truncation
      0.3, 0.001, 0.699,    # Second value needs truncation
      0.4, 0.4, 0.2),       # No truncation needed
    ncol = 3,
    byrow = TRUE
  )
