test_that("weight functions work with trimmed categorical propensity scores", {
  ...
  # Apply trimming
  trimmed_ps <- ps_trim(ps_matrix, .exposure = exposure, method = "ps", lower = 0.1)
...
test_that("weight functions work with truncated categorical propensity scores", {
  ...
  # Apply truncation
  truncated_ps <- ps_trunc(ps_matrix, .exposure = exposure, method = "ps", lower = 0.05)
...
test_that("weight functions work with data.frame propensity scores for categorical", {
  ...
  # Trim
  trimmed_ps <- ps_trim(ps_df, .exposure = exposure, method = "ps", lower = 0.15)
...
test_that("ATT weights work with categorical trimmed propensity scores", {
  set.seed(111)
  n <- 60
  exposure <- factor(sample(c("Control", "Treat1", "Treat2"), n, replace = TRUE))
  ...
  # Trim
  trimmed_ps <- ps_trim(ps_matrix, .exposure = exposure, method = "ps", lower = 0.2)
  
  # Calculate ATT weights for Treat1 as focal - expect warning about refitting
  expect_warning(
    wt_att_trimmed <- wt_att(trimmed_ps, .exposure = exposure, focal = "Treat1"),
    class = "propensity_no_refit_warning"
  )
...
test_that("weight functions work with refitted trimmed categorical propensity scores", {
  ...
  exposure <- factor(apply(probs, 1, function(p) sample(c("A", "B", "C"), 1, prob = p)))
